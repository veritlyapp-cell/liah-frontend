...
export async function POST(req: NextRequest) {
    try {
        ...
        console.log('[DEBUG] [WHATSAPP] Raw Webhook Body:', JSON.stringify(body, null, 2));
        ...
        if (body.object === 'whatsapp_business_account') {
            ...
            if (message) {
                ...
                if (type === 'text' || type === 'button') {
                    ...
                    Logger.info(`📍 Mapping origin: ${origin_id}`);
                    ...
                    Logger.info('🤖 Gemini result received');

                    // 3. Send response back via WhatsApp
                    if (result.response) {
                        Logger.info(`📤 Sending response to ${phone}: "${result.response.substring(0, 50)}..."`);
                        await sendWhatsAppMessage(phone, result.response);
                        Logger.success('📤 Response sent successfully');
                    } else {
                        Logger.warn('⚠️ No response generated by chatbot');
                    }
                } else {
                    Logger.warn(`⚠️ Received unsupported message type: ${type}`);
                }
            } else if (value?.statuses) {
                Logger.info('ℹ️ Received message status update (delivered/read)');
            } else {
                Logger.warn('⚠️ Received webhook without messages or statuses');
            }
