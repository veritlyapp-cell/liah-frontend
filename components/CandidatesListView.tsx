                                ...
                                <div className="flex items-start justify-between">
                                    <div className="flex-1">
                                        ...
                                        <div className="flex items-center gap-3 mb-2">
                                            ...
                                            {/* Other Active Processes Badge - STRICTLY filtered by holding */}
                                            {(() => {
                                                const holdingId = (claims as any)?.tenant_id;
                                                if (!holdingId) return null;
                                                const otherActiveApps = candidate.applications?.filter((app: any) =>
                                                    app.tiendaId !== storeId &&
                                                    app.holdingId === holdingId &&
                                                    app.status !== 'rejected' &&
                                                    app.hiredStatus !== 'hired' &&
                                                    app.hiredStatus !== 'not_hired'
                                                ) || [];
                                                if (otherActiveApps.length > 0) {
                                                    const otherStores = [...new Set(otherActiveApps.map((app: any) => app.tiendaNombre || app.marcaNombre))].join(', ');
                                                    return (
                                                        <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-purple-50 text-purple-700 border border-purple-200 flex items-center gap-1">
                                                            <span>📍</span> También en: {otherStores}
                                                        </span>
                                                    );
                                                }
                                                return null;
                                    ...
                                    <div className="flex flex-col gap-2 ml-4 min-w-[120px]">
                                        ...
                                        {latestApp && (latestApp.status === 'completed' || latestApp.status === 'interview_scheduled') && (
                                            <>
                                                <button
                                                    onClick={() => {
