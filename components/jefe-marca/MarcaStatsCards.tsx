...
interface MarcaStatsCardsProps {
    marcaId: string;
}
...
export default function MarcaStatsCards({ marcaId }: MarcaStatsCardsProps) {
        ...
        async function loadRQs() {
            ...
            try {
                ...
                rqs.forEach(rq => {
                    ...
                    newBreakdown.total++;
                    // Count unique stores
                    if (rq.tiendaId) uniqueStores.add(rq.tiendaId);

                    // Count by approval status (pending at jefe level = level 3)
                    const isApprovedByMe = rq.approvalChain?.some(a => a.level === 3 && a.status === 'approved');

                    if (rq.currentApprovalLevel === 3 && rq.approvalStatus === 'pending') {
                ...
                newBreakdown.stores = uniqueStores.size;
                setBreakdown(newBreakdown);
            } catch (error) {
                console.error('Error loading marca RQs:', error);
            } finally {
                setLoading(false);
            }
        }
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                ...
                <div className="bg-white rounded-lg shadow-lg p-6">
                    <p className="text-sm text-gray-600 mb-1">🏪 Tiendas Activas</p>
                    <p className="text-3xl font-bold text-blue-600">{breakdown.stores}</p>
                </div>
                <div className="bg-white rounded-lg shadow-lg p-6">
                    <p className="text-sm text-gray-600 mb-1">📊 Total RQs</p>
                    <p className="text-3xl font-bold text-gray-600">{breakdown.total}</p>
                </div>
            </div>
